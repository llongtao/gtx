...
public class RpcContext {
    ...
    private static String getAddressFromChannel(Channel channel) {
        ...
        if (socketAddress.toString().indexOf(GtxConstants.ENDPOINT_BEGIN_CHAR) == 0) {
            address = socketAddress.toString().substring(GtxConstants.ENDPOINT_BEGIN_CHAR.length());
        }
        return address;
    }
    ...
    private static Integer getClientPortFromChannel(Channel channel) {
        ...
        Integer port = 0;
        try {
            if (address.contains(GtxConstants.IP_PORT_SPLIT_CHAR)) {
                port = Integer.parseInt(address.substring(address.lastIndexOf(GtxConstants.IP_PORT_SPLIT_CHAR) + 1));
            }
